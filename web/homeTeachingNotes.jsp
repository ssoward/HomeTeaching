...
<%
  ...
  //message = (String)session.getAttribute("message");
   ...
   ArrayList<HTNotes> notesList = htn.fetchForFamPid(famPid);
 
%>
<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
  <form method="post" action="NewNote">
	<tr align=center>
  <td>Keep track of the <%=famm.getName()%> family!  <br><font size=2>Double click to clear screen.</font>
  </td>
  </tr>
	<tr align=center>
	<td colspan=2><textarea name="note" ondblclick="this.value=''" cols=35 rows=10><%=note%></textarea> </td>
  </tr>
	<tr align=center>
	<%
		if ( message != null ) {
	...
	<input type="hidden" name="famNote" value="true">
	...
	<tr>
        ...
        if(tNote.length()>10){
          ...
          <%}%>
				...
				<td>&nbsp;<a href="./homeTeachingNotes.jsp?compPid=<%=compPid%>&famPid=<%=temp.getFamPid()%>&notePid=<%=temp.getPid()%>">view</a></td>
				<td>&nbsp;<a href="./homeTeachingNotes.jsp?compPid=<%=compPid%>&famPid=<%=temp.getFamPid()%>&delNote=<%=temp.getPid()%>">remove</a></td>
			</tr>
			<%
			count++;           
            flip = !flip;
